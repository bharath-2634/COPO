...
const Overview = () => {
    ...
    //     }
    // }
    console.log(axiosInstance.defaults.baseURL + `/getSem-data/${encodeURIComponent(batchId)}`);
    ...
    const getSemesterData = async() => {
        ...
        try {
            ...
            console.log("Batch Data client side :", JSON.stringify(batchData, null, 2));
            if(response.data && response.data.batchData) {
                setBatchData(response.data.batchData);
            }
        }catch(error) {
    ...
    <div className='flex flex-col p-3 justify-between w-full h-full'>
        <div className='flex items-center justify-between w-full'>
            <div className='flex gap-3'>
                ...
                <select className='w-50 border rounded p-2 -mt-2' value={batchId} onChange={(e)=>{setBatchId(e.target.value)}}>
                    {
            ...
            <Link className='flex bg-green-600 p-2 text-white items-center gap-3 rounded' to="/dashboard/addSem" state={{batchId : batchId}}>
                <h2>Add </h2>
        ...
        <div className='mt-5 flex flex-col gap-5 '>
            ...
            {
                batchData.length===0 ? (<p> No Data found</p>):(
                    batchData.map((batch,idx)=>{
                        return (
                            <div>
                                <p className='bg-gray-300 p-2 w-20 rounded font-medium'>{batch.semName}</p>
                                            ...
                                            batch.subjects.map((subject,idx)=>{
                                                return (
                                                    <tr className="hover:bg-gray-100 border-b border-gray-200">
                                                        <td className="w-1/4 px-6 py-4">{subject.courseCode}</td>
                                                        <td className="w-1/4 px-6 py-4">{subject.courseName}</td>
                                                        <td className="w-1/4 px-6 py-4">{subject.staffName}</td>
                                                        <td className="w-1/4 px-6 py-4">{subject.status}</td>
                                                        <td className="w-1/4 px-6 py-4">{subject.category}</td>
