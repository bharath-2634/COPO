...
app.post("/save-student-data", async (req, res) => {
  try {
    const { studentsData } = req.body; // Extract the student data from the request body
    ...
    const studentPromises = studentsData.map(async (student) => {
      ...
      await newStudent.save(); // Save the student data
    });

    // Wait for all the students to be saved
...
app.get(
  "/getSubjectMapping/:batchId/:subjectCode/:subjectName",
  authenticateToken,
  async (req, res) => {
    const { user } = req.user;
    const { batchId } = req.params.batchId;
